...
class Spatial_Attention_Module(nn.Module):
    def __init__(self, k: int):
        ...
        # In order to keep the size of the front and rear images consistent with calculate, k = 1 + 2p,
        # k denote kernel_size, and p denote padding number so, when p = 1 -> k = 3; p = 2 -> k = 5; p = 3 -> k = 7,
        # it works. when p = 4 -> k = 9, it is too big to use in network
...
class CBAMBlock(nn.Module):
    def __init__(self, channel_attention_mode: str, channels: int = None, spatial_attention_kernel_size: int = 5,
        ...
        if channel_attention_mode == "FC":
            ...
            assert channels is not None and channel_attention_mode == "Conv", \
