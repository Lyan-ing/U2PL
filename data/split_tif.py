...
# 打开原始tif图像
def split_tif(input_file, output_prefix, tile_size):
    with rasterio.open(input_file) as src:
        ...
        for i in range(num_blocks_y):
            for j in range(num_blocks_x):
                ...
                if block.shape[0] == 1:
                    output_filename = f"{output_prefix}_{i}_{j}.png"
                    if block.max() > 0:
                        print()
                    img = block[0].astype(np.uint8)
                    img = Image.fromarray(img, mode="L")
                    img.save(output_filename)
                    continue
                # 获取小块的转换信息
                # transform = rasterio.windows.transform(window, src.transform)
                output_filename = f"{output_prefix}_{i}_{j}.tif"
                transform = src.window_transform(window)
                transform = rasterio.Affine(transform.a, transform.b, transform.c, transform.d, transform.e * -1,
                                            transform.f * -1)

                # 生成输出文件名

                # 保存当前小块为tif文件
                with rasterio.open(output_filename, "w", driver="GTiff", height=block.shape[1], width=block.shape[2],
                                   count=bands, dtype=block.dtype, crs=src.crs, transform=transform) as dst:
                    dst.write(block)
...
# 示例用法
input_file = r'E:\dataset\20231108_影像挑选\GF07_有云\GF7_DLC_W11.4_N11.4_20220622_L1A0000852849-BWDMUX.tiff'  # 输入的四波段TIFF图像文件名
output_prefix = r'E:\python\ZEV\U2PL\cloud\jpg\GF7'  # 输出小块的文件名前缀
tile_size = 512  # 小块的尺寸（宽度和高度）

split_tif(input_file, output_prefix, tile_size)
